...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 loss_type="l2",
                 ckpt_path=None,
                 ignore_keys=["model_ema.decay", "model_ema.num_updates"],
...
class LatentDiffusion(DDPM):
    ...
    def custom_loss(self, batch, loss_type = 0):

        closs = torch.nn.CosineSimilarity()
        ...
        img1_rec = self.predict_start_from_noise(img1_noisy, t=t, noise=img1_out)

        if loss_type == 0:
            loss = self.get_loss(img2_rec, img1, mean=False).mean([1, 2, 3])
        if loss_type == 1:
            loss = self.get_loss(img1_rec, img2, mean=False).mean([1, 2, 3])
        if loss_type == 2:
            loss = self.get_loss(img1_rec, img1, mean=False).mean([1, 2, 3])
        if loss_type == 3:
            loss = self.get_loss(img2_rec, img2, mean=False).mean([1, 2, 3])
    ...
    def make_centre(self, batch):
        ...
        z_C = (alpha)*z_S + (1-alpha)*z_R

        return z_C, alpha

    def cosine_loss(self, batch):
        criteria = nn.CosineSimilarity()

        z_C, alpha = self.make_centre(batch)
        caption = [str(alpha) + "-" + batch["base"]["caption"][0]]
        ...
        t = torch.randint(0, self.num_timesteps, (z_C.shape[0],), device=self.device).long()
        noise = default(None, lambda: torch.randn_like(z_C))
        x_noisy = self.q_sample(x_start=z_C, t=t, noise=noise)
        noise1 = self.apply_model(x_noisy, t, cond)
        pred = self.predict_start_from_noise(x_noisy,t,noise1)

        loss = criteria(pred, z_S).mean()

        return loss

    def base_loss(self, batch):
    ...
    def latent_shift(self, batch, optimizer_idx):
        ...
        noise = default(None, lambda: torch.randn_like(z_S))
        x_noisy = self.q_sample(x_start=z_S, t=t, noise=noise)
        noise_out = self.apply_model(x_noisy, t, cond)
        z_theta = self.predict_start_from_noise(x_noisy,t,noise_out)

        z_S_final = z_theta

        loss_base = self.get_loss(z_S_final, z_S, mean=False).mean([1,2,3])
        ...
        return loss_base, loss2

    def clip_loss(self, batch):
        
        text_tokens = clip.tokenize(["rabbit"]).cuda()
        ...
        t = torch.randint(0, self.num_timesteps, (z_R.shape[0],), device=self.device).long()
        logvar_t = self.logvar[t.cpu()].to(self.device)
        
        noise = default(None, lambda: torch.randn_like(z_R))
        x_noisy = self.q_sample(x_start=z_R, t=t, noise=noise)
        noise1 = self.apply_model(x_noisy, t, cond)
        z_theta = self.predict_start_from_noise(x_noisy,t,noise1)
        image = self.decode_first_stage(z_theta)

        image_features = self.clip_model.encode_image(image[:,:,0:224,0:224]).float()
        text_features = self.clip_model.encode_text(text_tokens).float()

        # image_features /= image_features.norm(dim=-1, keepdim=True)
        # text_features /= text_features.norm(dim=-1, keepdim=True)
        similarity = self.get_loss(image_features, text_features).mean()
    ...
    def make_images(self, batch):
        ...
        images = self.decode_first_stage(sample_scaled)
        images = torch.clamp((images+1.0)/2.0, min=0.0, max=1.0)

        img = 0
        for x_sample in images:
            x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
            name = "output_log/image"+str(self.iter)+"_"+str(img)+".jpg"
            Image.fromarray(x_sample.astype(np.uint8)).save(name)
            img += 1
    ...
    def validation_step(self, batch, batch_idx):
        return None
        batch_reg = batch["base"]
        _, loss_dict_no_ema = self.shared_step(batch_reg)
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch_reg)
            loss_dict_ema = {key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
    ...
    @torch.no_grad()
    def validation_step_prev(self, batch, batch_idx):
        batch_reg = batch["base"]
        # batch_fav = batch["favor"]
        _, loss_dict_no_ema = self.shared_step(batch_reg)
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch_reg)
            loss_dict_ema = {key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=100, quantize_denoised=False,

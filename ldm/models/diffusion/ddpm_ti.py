...
# if not self.done:
        ...
        #     with self.ema_scope():
        #         with torch.no_grad():
        #             cond = self.get_learned_conditioning(n_samples * ["rabbit"])
        #             ddim_sampler = DDIMSampler(self)
        #             shape = (self.channels, self.image_size, self.image_size)
        #             samples, intermediates =ddim_sampler.sample(200,n_samples,
        #                                                         shape,cond,verbose=False)
        #             self.style_samples = samples
...
class LatentDiffusion(DDPM):
    ...
    def custom_loss(self, batch, loss_type = 0):
        ...
        if loss_type == 3:
            loss = 1-abs(closs(img2_rec.flatten(start_dim=1), img1.flatten(start_dim=1)).mean())
        if loss_type == 4:
            loss = 1-abs(closs(img2_rec.flatten(start_dim=1), img2.flatten(start_dim=1)).mean())
